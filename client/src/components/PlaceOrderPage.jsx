...
//         setSelectedAddressId(addressRes.data.addresses[0]?._id || null);

//       } catch (error) {
//         console.error(error);
//         setError("Failed to fetch cart items or addresses.");
//       } finally {
//         setLoading(false);
//       }
//     };
...
//   } catch (error) {
//     console.error(error);
//     alert("Failed to remove item, rolling back...");
...
//         <Col md={7} className="py-4 px-md-5">
//           <OrderDetails
//             addresses={addresses}
//             cartItems={cartItems}
...
import LeftSidebar from "../components/LeftSidebar"; // Assuming path is correct
import OrderDetails from "../components/OrderDetails"; // Assuming path is correct
import PaymentSummary from "../components/PaymentSummary"; // Assuming path is correct
import api from "../utils/api";
import { useDispatch, useSelector } from "react-redux";
import { setCart } from "../redux/cartSlice";
...
export default function PlaceOrderPage() {
  ...
  const cartItems = useSelector((state) => state.cart.items);
  // Assuming user data might be in Redux state as well, or fetch separately
  ...
  useEffect(() => {
    ...
    if (token) fetchData(); else setError("User not authenticated.");
  }, [token, dispatch]);
  ...
  if (item.product) { // Safety check here
    return acc + item.product.price * item.quantity;
  }
  return acc; // Skip item if product data is missing
}, 0);
  const shippingFee = subtotal > 500 ? 0 : 40; // Example shipping logic
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const { data: { order } } = await api.post('/payment/create-order', {
        amount: total, // Send total amount in rupees
      },{
        headers:{
      ...
      const selectedShippingAddress = addresses.find(addr => addr._id === selectedAddressId);

      // Step 2: Configure Razorpay Options
      const options = {
        key: 'rzp_test_RF4l2HyfnUFssb', // Replace with your key ID
        amount: order.amount, // Amount in paise from backend response
        currency: 'INR',
        name: 'GadgetGrove',
        ...
        handler: async function (response) {
          ...
          const verificationData = {
            ...
            orderDetails: {
              items: cartItems.filter(item => item.product).map(item => ({
                ...
                image: item.product.image, // Pass image for order summary
          ...
          const verifyRes = await api.post('/payment/verify-payment', verificationData, {
             headers: { Authorization: `Bearer ${token}` },
          });
          ...
          try {
            ...
            dispatch(clearCart()); // Dispatch clearCart action from your slice
          } catch (cartClearError) {
            console.error("Failed to clear cart on backend:", cartClearError);
          }
           // Clear local cart state after successful order
          navigate(`/order/success/${verifyRes.data.order._id}`); // Redirect to order success page
        },
        prefill: {
  ...
  if (loading) return <div>Loading...</div>; // Replace with better spinner component
